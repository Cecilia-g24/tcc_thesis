"""
Approach 1 (English): LIWC -> human average_score regression with nested CV.

This script trains and evaluates LIWC-based regression models separately for each
transcultural-competence dimension, using the English LIWC results CSV. The
supervised target is `average_score`; MAE is the optimization goal used for
model selection.

Input CSV and output folder are both read directly from configs/paths.json
(data_clean / liwc_results_subdir / liwc_en_results_filename, and
results_approach1_en_dir). There is no file search/fallback.

Default output folder:
    results/approach1/en_results/mae_as_goal
...
INPUT_CSV = (
    ...
    / _PATHS["liwc_en_results_filename"]
)
DEFAULT_RESULTS_DIR = PROJECT_ROOT / _PATHS["results_approach1_en_dir"] / "mae_as_goal"
...
def detailed_metrics_from_permuter(
    ...
    if not metrics_df.empty:
        metrics_df = round_metric_columns(metrics_df.sort_values(["dimension", "MAE", "RMSE"]).reset_index(drop=True))
...
def export_combined_reports(
    ...
    best_per_dimension = (
        detailed_df.sort_values(["dimension", "MAE", "RMSE", "R2"], ascending=[True, True, True, False])
    ...
    best_family = (
        detailed_df.sort_values(["dimension", "model_family", "MAE", "RMSE", "R2"], ascending=[True, True, True, True, False])
    ...
    family_summary = (
        ...
        .sort_values(["dimension", "best_MAE"])
        .reset_index(drop=True)
    )
...
def save_metadata(
    ...
    metadata = {
        ...
        "selection_rule": "lowest MAE computed from outer-CV predictions",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Nested-CV LIWC (English) regression optimized for MAE.")
    parser.add_argument(
        "--results-dir",
        type=Path,
        default=DEFAULT_RESULTS_DIR,
        help="Output directory. Default: results/approach1/en_results/mae_as_goal",
    )
...
def run(args: argparse.Namespace) -> None:
    ...
    print("=" * 80)
    print("APPROACH 1 (ENGLISH): LIWC REGRESSION, MAE AS OPTIMIZATION GOAL")
    print("=" * 80)
    ...
    for dimension in dimensions:
        ...
        best = detailed_df.sort_values(["MAE", "RMSE", "R2"], ascending=[True, True, False]).iloc[0]
        print(
            f"Best by MAE: {best['model_family']} | "
